# Feature Specification: Nyx — prompt-to-DApp platform for Midnight Network

**Feature Branch**: `001-nyx-platform`
**Created**: 2026-07-10
**Status**: Draft
**Input**: User description: "Nyx: zero-trust multi-agent generative UI platform for Midnight Network developers — prompt-to-DApp (Compact contract + React frontend), compiled, tested, previewed in-browser, deployed to pre-prod. Ported from owner-approved discovery/SPEC.md"

> **Source of truth & provenance**: this specification is a faithful port of `discovery/SPEC.md`, produced by a full discovery process on 2026-07-10. Every requirement traces to a logged decision (D1–D62, `discovery/archive/DECISIONS.md`) or research finding (R1–R8, `discovery/archive/RESEARCH.md`); the ground-truth PRD is `.sdd/PRD.initial.md`. Do not re-decide settled decisions silently (PRD §16). Story 11 (BYOK) was descoped by the owner (D57).

**Terminology**: “compiler MCP” (PRD §8 vocabulary, used in Stories 1/4) and “toolchain MCP” (D30 vocabulary, used in Stories 2/8) name the **same service** — the owner's private, scale-to-zero Fly app that pins the compiler, exposes check/full compile modes, and holds the only R2 write credentials.
...
**Ground truth**: `.sdd/PRD.initial.md` — PRD-era settled decisions are D1–D18; the full decision log runs D1–D62 — none are re-decided silently.
...
- **Account**: keyed by the wallet's unshielded address (D43); owns projects, ledger balances, sessions, and provider of identity for every authorization check
- **Session**: server-side, 7-day sliding (D44), established by SIWE-style nonce signature; issues proving tokens (D52)
- **Project**: owned by an Account; lifecycle create/rename/soft-delete (30-day recovery, D49); exactly one live session (D40) and at most one active turn (D24) / in-flight deploy at a time
- **ProjectFile + Version**: `(project_id, path, content, version)` rows with turn-scoped transactional commits and retained version history (D26, D48); source of the manifest (paths + content hashes, D38)
- **ChatMessage**: conversation history persisted with the project and rehydrated on open (D23)
- **Turn**: one prompt→response agent cycle; carries reserve/settlement ledger linkage (D34), verify-cycle count (D21), and activity-stream events (D20)
- **LedgerEntry**: append-only log of deposits, reserves, settlements per Account; available + reserved balances derive from it (FR-043)
- **DepositRef**: random orchestrator-issued single-use reference binding an on-chain NyxtVault deposit to an Account (D45); expires if unseen; unmatched finalized deposits land in the Orphans table (D46)
- **DeployRegistryEntry**: `(project, address, version, status)`; exactly one active address per project (FR-057); drives teardown and the deletion cascade
- **ArtifactPrefix**: content-hashed R2 prefix per green compile holding prover/verifier keys, zkIR, and integrity manifest (D7, R3)
- **ProvingToken**: short-lived, session-bound credential for the hosted prover (D52)
- **CloneToken**: unguessable, revocable read-only credential for materialized git repos (D58)
- **ReconcileReport**: persisted record of each daily reconcile run — inputs, totals, burn action, outcome (D55, D56)
...
## Development Standards *(owner-selected, D61)*

- **DS-001**: Platform and generated-scaffold code MUST pass linting (ESLint + Prettier) and TypeScript strict-mode type checking in CI; no skipped or flaky checks (Constitution: Development Workflow gates)
- **DS-002**: Commits MUST follow Conventional Commits, enforced by pre-commit hooks that also run lint/typecheck/affected tests
- **DS-003**: The platform MUST validate its environment/config schema at boot and fail fast with named errors — covering the config-tunable surface (D19 routing, D47 economics, caps, cadences, thresholds)
- **DS-004**: Releases MUST follow the release-plz process used in Tome and Midnight Manual (verified against both repos' `release-plz.toml`, 2026-07-10): an automated front-end maintains a single "Release vX.Y.Z" PR on every push to main (version bump + changelog regenerated from Conventional Commits, git-cliff grouping: feat→Added, fix→Fixed, perf→Performance, refactor→Changed, docs→Documentation, revert→Reverted, chore(deps)→Dependencies, noise skipped, catch-all→Other, breaking commits protected); merging that PR is the single approval gate and pushes the `vX.Y.Z` tag; the tag fires a back-end release workflow that owns builds, the GitHub Release, and any publishing — with publishing gated behind green builds (Tome's `git_only` tags-as-truth idiom for single artifacts; mnm's `version_group` single-tag idiom if a workspace emerges). Tooling: release-plz itself for Rust components (D33 crates); the same process mirrored for the TypeScript platform (tooling chosen at implementation)

## Assumptions, Gates & Watching Items

**Hard pre-implementation gates** (recorded in the decision log; implementation MUST NOT start on the gated story until cleared):

- **Story 9 — Q3 injection gate (D54)**: the R6 PoC live run must confirm Lace injects `window.midnight` into the top-level preview origin. PoC ready at `pocs/webcontainer-lace/` (branch `worktree-agent-ab47e5ba8f8087738`).
- **Story 6 — vault-funding spike (R4)**: prove Lace/`balanceUnsealedTransaction` can fund a contract-side `receiveUnshielded` end-to-end on pre-prod (~1 day).
- **Story 10 — burn circuit design**: designed at implementation via mnm/MNE, never from memory (D55, PRD §16).
- **Story 8 — on-chain teardown semantics**: verify via mnm whether superseded contracts support any on-chain disable.

**Standing Watching items**:

- Proving flip-back (D37): when the upstream wallet-sdk tx-history migration fix lands (R8), retest in-wallet proving and flip the scaffold default from the hosted prover.
- Hollow-test greens (D41): per-circuit coverage telemetry (FR-032) provides the evidence base to add a mechanical adequacy floor via revision if needed.
- Real-value-network revisits: hosted-prover privacy posture (D37), session lifetime (D44), deploy pricing (D51).
- WebContainer license recheck at commercial launch (D29); compiler version-bump cadence (D6).

**Assumptions held by owner fiat** (owner develops the upstream tools): Tome retrieval works (D27); mnm hosted MCP reachable (D28); toolchain MCP available to agents (D30, D31).
