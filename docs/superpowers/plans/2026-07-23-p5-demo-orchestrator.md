...
Set the top-level `name: nyx-demo`. Check `apps/server/src/config/schema.ts` for the exact override var names (`NYX_NODE_URL`/`NYX_INDEXER_URL` exist today; verify nothing was renamed by P2–P4). Three read/write mounts on `nyx-server` (P2 + P3 driven): the SRS cache (`- ./srs-cache:/srs-cache` with `SRS_CACHE_DIR=/srs-cache` in the container env — Task 9's srs phase downloads into `infra/demo/srs-cache/`, gitignored), the artifact store (`- nyx-demo-artifacts:/artifacts` named volume with `ARTIFACT_STORE_ROOT=/artifacts`, so artifacts survive container rebuilds), and the vault key material (`- ./vault-artifacts:/vault-artifacts` read-only with `VAULT_ARTIFACTS_DIR=/vault-artifacts` — Task 8 copies the compiled vault `keys/` + `zkir/` there; the P3 `GET /vault-artifacts/*` route serves it to the browser ceremony); add `nyx-demo-artifacts` to the volumes block and `infra/demo/srs-cache/` + `infra/demo/vault-artifacts/` to `.gitignore` in Step 3.
...
- [ ] **Step 1: Derive the authoritative var list.** Open `apps/server/src/config/schema.ts` (post-P3/P4 state) and enumerate every REQUIRED var + every demo-relevant optional. Cross-check against Task 1 Step 4's captured fail-fast output. **Verified against `schema.ts` (EnvSchema, `schema.ts:113-211`) at P3 Task 0 — REQUIRED: `DATABASE_URL`, `MCP_TOME_URL`, `MCP_MNM_URL`, `PROVER_URL`, `DEPLOY_KEY`, `MODEL_ROUTING`; everything else is optional-with-default** (`PORT` defaults 8080). Demo set: `DATABASE_URL` (localhost:5433 for host-run tools; the compose in-network override handles the container), `PROVER_URL=http://localhost:6300`, `NYX_NETWORK=local-devnet`, `DEPLOY_KEY` (from `state.deployKeySeed`), `MODEL_ROUTING` (operator), one or more LLM API keys (operator), plus the P2/P3-introduced keys (defaulted/optional — set explicitly in `.env.demo` so the demo is self-describing): `PUBLIC_ORIGIN`, `ARTIFACT_STORE_ROOT`, `ARTIFACT_MAX_FILE_BYTES`, `ARTIFACT_MAX_BUNDLE_BYTES`, `COMPILE_CHECK_TIMEOUT_MS`, `COMPILE_FULL_TIMEOUT_MS`, `SRS_CACHE_DIR` (gates the `/srs/*` route — registered only when set, `app.ts:208-211`), and the P3 additions `VAULT_ARTIFACTS_DIR` (gates `/vault-artifacts/*` — must point at the vault build dir Task 8 produces, so the browser ceremony can fetch prover/verifier keys + `.bzkir`) + `DEPOSIT_POLL_INTERVAL_MS` (confirm final names against the P3 retro); economic tunables left to defaults. `MCP_TOOLCHAIN_URL` no longer exists (compact-mcp removed in P2) — it must NOT appear in `.env.demo` or the example file. Web set: `VITE_DEV_WALLET=1`, `VITE_DEV_WALLET_SEED` (from `state.userWalletSeed`), `VITE_NYX_NETWORK=local-devnet`, `VITE_NYXT_VAULT_ADDRESS` (P3 adds it to the `apps/web/src/config.ts` chokepoint — confirm the export as merged; the address must flow ONLY via the config chokepoint, constitution VII).
