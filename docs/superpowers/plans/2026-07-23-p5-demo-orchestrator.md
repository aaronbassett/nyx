...
Set the top-level `name: nyx-demo`. Check `apps/server/src/config/schema.ts` for the exact override var names (`NYX_NODE_URL`/`NYX_INDEXER_URL` exist today; verify nothing was renamed by P2–P4). Three read/write mounts on `nyx-server` (P2 + P3 driven): the SRS cache (`- ./srs-cache:/srs-cache` with `SRS_CACHE_DIR=/srs-cache` in the container env — Task 9's srs phase downloads into `infra/demo/srs-cache/`, gitignored), the artifact store (`- nyx-demo-artifacts:/artifacts` named volume with `ARTIFACT_STORE_ROOT=/artifacts`, so artifacts survive container rebuilds), and the vault key material (`- ./vault-artifacts:/vault-artifacts` read-only with `VAULT_ARTIFACTS_DIR=/vault-artifacts` — Task 8 copies the compiled vault `keys/` + `zkir/` + `contract/` there; the P3 `GET /vault-artifacts/*` route serves keys/zkir to the browser ceremony, and the server-side P4 `DepositsStateReader` dynamic-imports `contract/index.js` from the same dir); add `nyx-demo-artifacts` to the volumes block and `infra/demo/srs-cache/` + `infra/demo/vault-artifacts/` to `.gitignore` in Step 3.
...
- Consumes: `packages/nyxt-vault` build — **script name verified at P4 Task 0: `compact:build`** (`packages/nyxt-vault/package.json:7` = `compact compile src/nyxt-vault.compact build/nyxt-vault`; there is NO plain `build` script; `typecheck`/`test` guard on `command -v compact`, `compact:build` itself does not — a missing CLI fails with the exec error, which the phase maps to the install hint), the P4-verified deploy path (`createDevnetDeployExecutor` deps or the lower-level SDK calls its retro documents), state store.
- Produces: `createVaultPhase(deps): Phase` — deps inject `{ compileVault(): Promise<{buildDir: string}>, deployVault(deployKeySeed: string): Promise<{contractAddress: string}> }` (`VaultOps` seam + real adapter). Satisfied when `state.vaultAddress` set AND a live existence probe passes (indexer lookup of the address — reuse the probe P4's `awaitFinality` work established). Writes `vaultAddress` to state; the design's constitution-VII flow delivers it to web env in Task 10. **P3/P4 seam obligation:** the compiled vault's `keys/{deposit,burn}.{prover,verifier}` + `zkir/*.bzkir` **+ `contract/index.js`** (the native compile emits all three, SPIKE-2 §B; build dir `packages/nyxt-vault/build/nyxt-vault/`) must end up where the server's `VAULT_ARTIFACTS_DIR` points (env generation, Task 10, + the compose mount when containerized): the browser top-up ceremony fetches keys/zkir from the P3 `GET /vault-artifacts/*` route, and the server's P4 Task 3b `DepositsStateReader` dynamic-imports the compiled `contract/index.js` for the on-chain `deposits`-map decode. Record the `buildDir` (or a copied stable dir under `.state/`) in state so env-gen can reference it.
...
- [ ] **Step 2: Run — FAIL. Step 3: Implement** — real `compileVault` = `ctx.exec("sfw", ["pnpm", "--filter", "@nyx/nyxt-vault", "compact:build"], ...)` (script name verified — the compact-CLI local build per design §2 "our code compiles on our machine"; confirm the package's workspace name matches the filter before wiring) then copy `keys/` + `zkir/` + `contract/` to the stable vault-artifacts dir; real `deployVault` reuses the P4 executor path with the vault's compiled artifacts — the P5 Task 0 re-planning has aligned the exact call; do NOT invent SDK calls here.
- [ ] **Step 4: PASS + gates.**
...
- [ ] **Step 1: Derive the authoritative var list.** Open `apps/server/src/config/schema.ts` (post-P3/P4 state) and enumerate every REQUIRED var + every demo-relevant optional. Cross-check against Task 1 Step 4's captured fail-fast output. **Verified against `schema.ts` (EnvSchema, `schema.ts:113-210`) at P4 Task 0, post-P3 merge — REQUIRED: `DATABASE_URL`, `MCP_TOME_URL`, `MCP_MNM_URL`, `PROVER_URL`, `DEPLOY_KEY`, `MODEL_ROUTING`; everything else is optional-with-default** (`PORT` defaults 8080). Demo set: `DATABASE_URL` (localhost:5433 for host-run tools; the compose in-network override handles the container), `PROVER_URL=http://localhost:6300`, `NYX_NETWORK=local-devnet`, `DEPLOY_KEY` (from `state.deployKeySeed`), `MODEL_ROUTING` (operator), one or more LLM API keys (operator), plus the P2/P3-introduced keys (defaulted/optional — set explicitly in `.env.demo` so the demo is self-describing; ALL names confirmed in the merged `schema.ts`): `PUBLIC_ORIGIN` (`:121`), `ARTIFACT_STORE_ROOT`, `ARTIFACT_MAX_FILE_BYTES`, `ARTIFACT_MAX_BUNDLE_BYTES`, `ARTIFACT_MAX_STAGED_BYTES_PER_PROJECT`, `ARTIFACT_MAX_STAGED_PREFIXES_PER_PROJECT` (`:133-140`), `COMPILE_CHECK_TIMEOUT_MS`, `COMPILE_FULL_TIMEOUT_MS` (`:142-143`), `SRS_CACHE_DIR` (`:145`; gates the `/srs/*` route — registered only when `config.artifacts.srsCacheDir` is set, `app.ts:215-219`), `VAULT_ARTIFACTS_DIR` (`:149`; gates `/vault-artifacts/*` via `config.artifacts.vaultArtifactsDir`, `app.ts:221-227` — must point at the vault dir Task 8 produces with `keys/` + `zkir/` + `contract/`), `DEPOSIT_POLL_INTERVAL_MS` (`:203`, default 5000), and `NYXT_VAULT_ADDRESS` (`:210` — NOTE: defaults to `""`, and boot passes `config.nyxtVaultAddress` into the observation query; the demo MUST set it from `state.vaultAddress` or the deposit poller reads an empty address); economic tunables left to defaults. `MCP_TOOLCHAIN_URL` no longer exists (compact-mcp removed in P2) — it must NOT appear in `.env.demo` or the example file. Web set: `VITE_DEV_WALLET=1`, `VITE_DEV_WALLET_SEED` (from `state.userWalletSeed`), `VITE_NYX_NETWORK=local-devnet`, `VITE_NYXT_VAULT_ADDRESS` (confirmed as merged: read defensively at `apps/web/src/config.ts:86-101`, exported as the chokepoint constant `NYXT_VAULT_ADDRESS` — the address must flow ONLY via the config chokepoint, constitution VII).
...
- [ ] **Step 1: Failing test** — `parseArgs`: `[]`→up, `["--reset"]`→reset, `["--check"]`→check, unknown flag → error naming it. `buildPhases("up")` returns phases in EXACTLY this order: preflight → compose-up(devnet trio + postgres) → devnet-health → funding → vault → migrate → srs → env-gen → compose-build+up(nyx-server) → server-health (`waitForHttpOk` on `http://localhost:8080/health` — **path verified at P3 Task 0**: `apps/server/src/http/health.ts:31` registers `GET /health` (liveness) and `GET /ready` (readiness incl. a DB probe) — prefer `/ready` for the post-boot wait since it proves Postgres wiring) → mcp → vite (spawn `sfw pnpm --filter @nyx/web dev`, wait on 5173) → print-url.
...
- [ ] **Step 1: Full cold run.** `pnpm demo -- --reset && pnpm demo`. Expected: every phase runs, ends with the URL printed. Record wall-clock per phase in the retro. (Scope note, so the P2-retro obligation is not read as dropped: the `getSources`+`projectId` → `createPreview` wiring that registers `compile:run` handling is WEB composition and is discharged by P6 Task 11 — this cold run proves stack liveness, not the turn-compile loop.)
