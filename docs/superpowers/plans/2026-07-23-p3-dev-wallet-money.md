...
- ⚠️ One version-string discrepancy between the reports: SPIKE-1 §5 pins `wallet-sdk@1.1.0`, SPIKE-2 risk 2 says `wallet-sdk 1.0.0`. Resolve from the installed package at implementation (`npm view` + the spike workspace lock); do not guess.
...
- Modify: `apps/server/src/app.ts` (there is no `server.ts` — `buildServer` lives in `app.ts`; register the routes where `registerProverRoutes` is registered, `app.ts:263`, inside the auth-gated block, and mirror how `requireSession` is built at `app.ts:205`)
...
- **⚠️ WS legs (discovered from the SPIKE-2 recipe — a plain HTTP forwarder is NOT enough):** the verified wallet stack needs `indexerWS` (`ws://…:8088/api/v4/graphql/ws` — the http path 405s WS upgrades) and `nodeWS` (`ws://…:9944`) alongside the HTTP endpoints. The isolated page cannot open those cross-origin sockets either. So this task must ALSO relay WebSocket upgrades on the two prefixes (the repo already ships `@fastify/websocket` — a thin session-gated socket-pair relay per prefix, bytes forwarded verbatim both ways, close propagated), OR — if at implementation the ceremony stack demonstrably runs with HTTP-only endpoints (probe, don't assume; `MidnightWalletProvider` sync and `submitTransaction` both name WS endpoints) — record that evidence in the retro and skip the relay. Decide from execution, never from memory (constitution I).
...
- Consumes: the vendored zkir wasm from `@nyx/compact-wasm` (read `packages/compact-wasm/src/` AS MERGED — if P2 shipped no prover/keygen surface beyond the compiler, the zkir `provingProvider` wiring lands HERE over the vendored zkir bundle, per the P2 retro's keygen-gate outcome), the extended `/prover/*` proxy, the artifact read path (`@/artifacts`) for key material.
...
- [ ] **Step 3: Implement both adapters + the proxy subpath relay.** Proxy adapter mirrors `auth.ts` transport idioms (injectable `fetch`/`baseUrl`). Wasm adapter wraps the vendored zkir `provingProvider` per its REAL exports (read the installed/vendored module first — constitution I).
...
- [ ] **Step 1 (constitution I — GATE): assemble the verified tx recipe.** The executed, devnet-accepted recipe is in `SPIKE2_REPORT.md` and summarized in this plan's verdict block: pins `midnight-js-*@4.1.1` + `ledger-v8@8.1.0` + `testkit-js@4.1.1` + `zkir-v2@2.1.0` (wallet-sdk version: resolve the 1.0.0-vs-1.1.0 report discrepancy from the installed package); `setNetworkId('undeployed')` (lowercase — capitalized is node rejection 1010/Custom 166); assembly via `midnight-js-contracts` `ContractCallPrototype`/`Transaction.fromPartsRandomized`; prove via `tx.prove(provingProvider, CostModel.initialCostModel())` (Task 4 factory); balance/sign/finalize via the wallet-sdk facade (`balanceUnboundTransaction` → `signRecipe` → `finalizeRecipe`); submit via `wallet.submitTransaction` (node WS relay — through the Task 1 forwarding, never a direct devnet URL); observe via indexer `watchForTxData`. **The wallet's fee leg (DUST spend) is proven by the wallet's configured prover** — point it at the proof server via the Task 4 `/prover/*` proxy (or `wallet-sdk-prover-client` `WasmProver` if chosen; record which). The dev wallet must be NIGHT-funded + DUST-registered BEFORE the first ceremony (P5's funding phase / the spike fixture — §G). Then: (2) read the installed `.d.ts` of every package the recipe names; (3) if ANY step is still ambiguous, dispatch `midnight-verify` (`/midnight-verify:verify`) with the specific claim and wait for Confirmed before coding. Record the final recipe as a comment block at the top of `dev-ceremony-tx.ts` with citations (report section / verify transcript). DO NOT write `dev-ceremony-tx.ts` bodies before this step is complete.
...
- [ ] **Step 7: Wire the topup construction site** (`topup.tsx` wiring helper): when `maybeInstallDevWallet`-style env gating is active (`VITE_DEV_WALLET === "1"`), construct `createDevWalletCeremony` with the WASM prover as primary and the proxy prover as the designed fallback (SPIKE-2 verdict block); otherwise keep `createOwnerGatedCeremony`. **Proving-budget UX (SPIKE-2 risk 1):** the ~23–26 s prove must surface as an honest in-progress state in the topup state machine (read `topup.tsx`'s existing states — extend, don't bypass), run in the Task 4 Web Worker, and PREFETCH key material (prover key 2.8 MB + IR + SRS) when the modal opens rather than at prove time. **Serialize submissions per wallet** (SPIKE-2 risk 7): the ceremony must queue, never parallelize, submissions from the dev-wallet seed. Run the full web suite. Expected: PASS.
- [ ] **Step 8: Commit.**
