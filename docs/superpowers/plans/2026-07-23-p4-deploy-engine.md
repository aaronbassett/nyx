...
**Tech Stack:** TypeScript/Node ≥22, Fastify 5, vitest, Midnight SDK (`@midnight-ntwrk/*` at SPIKE-1-pinned versions — every shape verification-gated), devnet (node `0.22.5` :9944, indexer `4.2.1` :8088, proof-server `8.1.0` :6300), `@nyx/scaffold` steering.
...
- [ ] **Step 1: Collect existing evidence.** Read, in order: `docs/superpowers/plans/retros/SPIKE1_REPORT.md` (a wasm-compiled contract was deployed + executed on this devnet — its deploy path is the closest prior art), `SPIKE2_REPORT.md` + `P3_RETRO.md` (verified submit + indexer recipes), and the P5-adjacent vault bootstrap if P1/P5 landed one (`grep -rn "deploy" infra/demo/ packages/nyxt-vault/ --include="*.ts" -l`). List which of the five recipe elements (build, prove, sign+submit, finality, balance) each source pins.
...
- [ ] **Step 3: Implement `devnet-executor.ts`** (orchestration + classification only — zero `@midnight-ntwrk` imports in this file; injected `delay`/`now` defaulting like `pipeline.ts` does — read its `DeployPipelineDeps` defaults and mirror).
...
- Consumes: `BalanceQuery = () => Promise<bigint>` (`deploy/wallet.ts:43` — base units; a rejection is the loud "unavailable" that makes `assertCanDeploy` fail closed), the `sdk-recipe.md` balance element.
...
- Consumes: the current wiring at `index.ts:169-201` — `createOwnerGatedDeployExecutor({signingKey: config.secrets.deployKey, network: config.network, proverClient})`, the rejecting `queryBalance` stub, and `getLatestGreenBuild: () => Promise.resolve(null)`.
- Produces: real `createDevnetDeployExecutor` + `createDevnetBalanceQuery` wired; `getLatestGreenBuild` wired to the P1 green-build store method (per `P1_RETRO.md` — if P1 already wired it, verify and leave it).
...
- [ ] **Step 3: Verify `getLatestGreenBuild`** is wired to the P1 store method (`recordGreenBuild`/`getLatestGreenBuild` per the canonical registry). If `index.ts` still stubs `null`, wire it now: `getLatestGreenBuild: (projectId) => greenBuildStore.getLatestGreenBuild(projectId)` matching the handler's expected shape (read `deploy/handler.ts` for the exact signature).
...
- Produces: `SCAFFOLD_STEERING.packageManagerRule` (id `"package-manager"`) and `SCAFFOLD_STEERING.devWalletRule` (id `"dev-wallet"`), appended to `SCAFFOLD_STEERING_RULES` (order-stable list — append, don't reorder). The agents' instruction builders (`instructions.ts`) compose all rules automatically iff they iterate `SCAFFOLD_STEERING_RULES` — verify by reading `instructions.ts:76-110`; if they enumerate rules by name, add the two new ones there too.
