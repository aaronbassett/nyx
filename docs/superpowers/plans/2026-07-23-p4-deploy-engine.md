...
- Consumes: the current wiring at `index.ts:166-209` — `createOwnerGatedDeployExecutor({signingKey: config.secrets.deployKey, network: config.network, proverClient})` (`index.ts:172-176`), the rejecting `queryBalance` stub (`index.ts:181-186`), and `getLatestGreenBuild` — **already REAL: P1 wired `(projectId) => projectStore.getLatestGreenBuild(projectId)` at `index.ts:204`** (store method `projects/store.ts:511-518`, migration 0005).
...
- [ ] **Step 3: Verify `getLatestGreenBuild`** is still the P1 wiring (`getLatestGreenBuild: (projectId) => projectStore.getLatestGreenBuild(projectId)`, `index.ts:204`, returning `DeployArtifacts | null` — `{urlPrefix, compilerVersion}`, `deploy/pipeline.ts:71-76`). Nothing to add; a regression here is a red flag, stop and investigate.
