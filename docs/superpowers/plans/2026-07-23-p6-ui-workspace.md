...
- Consumes: `resumeSession`, `signIn`, `Account` from `@/wallet/auth`; `useWalletConnect` from `@/wallet/useWalletConnect`; the P3 dev wallet (P3 wires `maybeInstallDevWallet()` in `apps/web/src/main.tsx` before first render, so detection finds `window.midnight.nyxDev` when `VITE_DEV_WALLET=1` — confirm as merged via the P3 retro).
...
- Consumes: `Chat` + `ChatBridge` + `useTurnState`/`TurnState` from `@/chat`; `BalanceCard`, `EntryFeed`, `LowBalanceNudge`, `useLedger`, `formatNyxt` from `@/ledger`; `TopUp` (props = `UseTopUpOptions` seams: `DepositClient`, `DepositCeremony` ← P3's env-gated selector `selectDepositCeremony` (`@/wallet` — P3 ships it because `topup.tsx` has no construction site; it returns `createDevWalletCeremony` under `VITE_DEV_WALLET=1`, else the owner-gated stub), `DepositSubscription`, `TopUpClock`) from `@/wallet/topup`; a shadcn `Dialog` (add `components/ui/dialog.tsx` via the existing shadcn pattern if not present — copy the idiom of `components/ui/button.tsx`, no CLI).
...
- Consumes: `FileChangedEventSchema` payload (`@nyx/protocol` `events.ts:400-410`, schema at `:409` — already defined, currently UNHANDLED server-side; anchors re-verified post-P2, the P2 compile events shifted this region); `ProjectStore.commit(projectId, request)` (`store.ts:102`; `CommitRequest` at `store.ts:57-60` is `{ author: FileAuthor; files: readonly FileWrite[] }` — the user-edit marker is `author: "user"`, the D59 source distinction); `getVersionHistory` (`store.ts:110`) returning `VersionSnapshot[]` (`store.ts:78-85`: `{ version, author, createdAt, files: HandoffFile[] }` — filter `author === "user"`); the per-project turn-active state in the coordinator (`TurnGate.isTurnActive`, interface at `coordinator.ts:285`, active-turn tracking near `:516`; handler registration site: the `.on("prompt:submit", …).on("test:results", …).on("compile:results", …)` chain at `coordinator.ts:1026-1042` — register `file:changed` alongside).
- Produces:
...
- Consumes: `dirtyPaths`/`EditorState` (Task 9); the bridge `send` for `file:changed` and `deploy:request` (payload `{}` — `events.ts:394-397`, re-anchored post-P2) and `deploy:status` subscription; P2's `CompileWorkerClient.check(sources)` for Build — **surface verified at P3 Task 0**: `createCompileWorkerClient` from the `@/compile` barrel (`compile/client.ts:32-39`: `check(sources): Promise<CheckOutput>` / `compileFull` / `dispose`; `CheckOutput` at `compile/messages.ts:16`, `FullOutput` extends it at `:28`, both carrying `compilerVersion` + `durationMs` from `@nyx/compact-wasm`'s `CheckResult`/`CompileResult`); diagnostics → monaco markers via `monaco.editor.setModelMarkers`. Timing/version for the pitch chip come from the compile result itself — P2's `compile:results`/worker results carry `durationMs` and `compilerVersion` (plus optional `circuits`); render `durationMs` as seconds and show `compilerVersion` in the chip tooltip. Never measure with `Date.now()` around the call when the result already reports its own duration.
- Produces:
