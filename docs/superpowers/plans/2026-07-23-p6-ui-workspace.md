...
- Consumes: `resumeSession`, `signIn`, `Account` from `@/wallet/auth`; `useWalletConnect` from `@/wallet/useWalletConnect`; the P3 dev wallet — **as merged (`330ea90`): `main.tsx` calls `bootApp` (`apps/web/src/boot.ts:38`) which, when `VITE_DEV_WALLET=1`, DEFERS the first render until the dynamic dev-wallet chunk imports + `maybeInstallDevWallet()` installs `window.midnight.nyxDev`** (detection is a ONE-SHOT synchronous mount snapshot — no polling — so any new mount-time detection this task adds inherits the same ordering guarantee; a failed chunk load logs and renders without the wallet).
...
- Consumes: `Chat` + `ChatBridge` + `useTurnState`/`TurnState` from `@/chat`; `BalanceCard`, `EntryFeed`, `LowBalanceNudge`, `useLedger`, `formatNyxt` from `@/ledger`; `TopUp` (props = `UseTopUpOptions` seams (`topup.tsx:425-461`): `DepositClient`, `DepositCeremony` ← P3's env-gated selector `selectDepositCeremony(deps: SelectCeremonyDeps)` (`@/wallet`, `ceremony-select.ts:107-144` as merged — it REQUIRES a `signer: DevSigner` (build via `createDevSigner(seed, network)`, `dev-signer.ts:136`, seed from `VITE_DEV_WALLET_SEED`); returns `createDevWalletCeremony` under `VITE_DEV_WALLET=1`, else the owner-gated stub), `DepositSubscription` (real adapter from Task 6), `TopUpClock`) from `@/wallet/topup`; a shadcn `Dialog` (add `components/ui/dialog.tsx` via the existing shadcn pattern if not present — copy the idiom of `components/ui/button.tsx`, no CLI). **⚠️ P3 review carry (Opus-3, recorded in the P3 retro for THIS plan): memoize the `selectDepositCeremony` result per connected wallet/signer (`useMemo` keyed on the signer identity)** — the selector wraps the submitter in `serializeSubmissions` (the SPIKE-2 risk-7 per-wallet serialization), and re-running it every render resets that serialization queue.
...
### Task 5b: Real deposit ceremony — un-gate the P3 SDK legs (web)

**Added at P4 Task 0 (2026-07-24), grounded in the P3 retro (Opus-4) + merged code — without this the golden-path top-up beat REJECTS:** P3 shipped the full ceremony orchestration but every SDK leg is an owner-gated rejecting stub: `createDepositTxBuilder` / `createDevnetSubmitter` (`wallet/dev-ceremony-tx.ts:122/:184`) reject with a message naming exactly the missing packages ("install `@midnight-ntwrk/midnight-js-contracts@4.1.1` + `wallet-sdk@1.1.0`"), and `selectDepositCeremony`'s default provers are rejecting stubs (`ceremony-select.ts` `ownerGatedProver`). The P3 retro records verbatim: "nothing yet constructs a `CircuitKeySource` over `/vault-artifacts` + `/srs` or composes `createTxCeremonyProver(createWasmCeremonyProver, keySource)` into the selector — P5/P6 must write that wiring, not just install the SDK packages." P5 provides the env/vault/artifacts; the web composition lands HERE. By this point the SPIKE-proven matrix is installed server-side (P4) and the vault is deployed with served key material (P5).

**Files:**

- Modify: `apps/web/package.json` (add `@midnight-ntwrk/midnight-js-contracts@4.1.1` + `@midnight-ntwrk/wallet-sdk@1.1.0` — `ledger-v8@8.1.0` + `zkir-v2@2.1.0` are ALREADY deps; versions are the SPIKE-pinned matrix, `sfw pnpm --filter @nyx/web add …`; a <7-day-old release would trip `minimumReleaseAge` — these pins are old, but if refused add a justified scoped exclude per P0 retro, never lower the global age)
- Modify: `apps/web/src/wallet/dev-ceremony-tx.ts` (real `createDepositTxBuilder` + `createDevnetSubmitter` bodies), `apps/web/src/wallet/ceremony-select.ts` (real default provers), plus a new `apps/web/src/wallet/key-source.ts` (fetch-based `CircuitKeySource`)
- Test: extend `apps/web/tests/wallet/` ceremony suites (deterministic over fakes — the existing owner-gated tests assert the rejection messages; they change to assert the real seams' orchestration with injected fakes)

**Interfaces:**

- Consumes: the SPIKE-2 §What-a-tx-needs pipeline (unproven-tx assembly via `midnight-js-contracts` → `tx.prove` seam → wallet facade `balanceUnboundTransaction` → `signRecipe` → `finalizeRecipe` → submit; every shape midnight-verify-gated at implementation, constitution I — the P4 retro's `sdk-adapter.ts` evidence is the server-side sibling); `CircuitKeySource`/`createWasmCeremonyProver`/`createProxyCeremonyProver`/`createTxCeremonyProver` (`wallet/ceremony-prover.ts:68/:145/:225/:156` — the seams are DESIGNED for this, body-only composition); the P3 same-origin routes: `GET /vault-artifacts/*` (per-circuit `{proverKey, verifierKey, ir}`), `GET /srs/*` (SRS params), and the `/devnet/node/*`(+WS relay)/`/devnet/indexer/*` forwarders (P3 retro: `wallet.submitTransaction` uses the node WS transport — the WS relay is NECESSARY, a raw HTTP forward is insufficient); lowercase `undeployed` tx-encoding id (`DEV_TX_NETWORK_ID`, `dev-ceremony-tx.ts:83`).
- Produces: a `selectDepositCeremony` composition whose defaults are REAL — wasm prover (primary) over the fetch-based `CircuitKeySource`, proxy prover (fallback) via the same-origin `/prover/*` proxy, real buildTx/submit over the forwarders — while every seam stays injectable so the deterministic suites keep running against fakes. Proving budget ~23–26 s at k=13 (SPIKE-2): keep the ceremony's progress states honest and run the prove off the main thread (`CeremonyWorkerRunner` seam exists, `ceremony-prover.ts:127`).

- [ ] **Step 1: Verify-then-code (constitution I).** Read the installed `.d.ts` of both new packages + the P4 retro recipe; dispatch `midnight-verify` for any call shape neither pins. No SDK line from memory.
- [ ] **Step 2: TDD the key source + composition** (fetch fakes: keys/ir fetched per circuit from `/vault-artifacts`, SRS from `/srs`, cache-once semantics; composition test: `selectDepositCeremony` defaults resolve to the real legs under `VITE_DEV_WALLET=1` with fakes injected at the network boundary only).
- [ ] **Step 3: Implement the real bodies**, keeping failure-as-rejection semantics (`CeremonyProvingError` stages) and the serialized submitter. Run web gates.
- [ ] **Step 4: Live proof** is the golden-path rehearsal beat 4 (real deposit → pending → credited against the running demo stack) — record the observed prove time in the retro.
- [ ] **Step 5: Commit** — `git commit -m "feat(web): real deposit ceremony over vault artifacts and devnet relay"`.
...
- Consumes: `LedgerBridge` type from `@/ledger/types` (`types.ts:59`, `{ on(type, handler): Unsubscribe }` over `LedgerBridgeEventType = "ledger:update" | "turn:settled"`, `types.ts:50`); `PreviewBridgeConnection` from `@/container/ws-client` (`ws-client.ts:86`, structural `send`/`on`); the P3 `deposit:failed` S→C event (`@nyx/protocol` `events.ts:274-292` — added by P3 with NO web consumer yet; the P3 retro names topup's `DepositSubscription` as its intended consumer) and the `DepositSubscription` seam (`@/wallet/topup` `topup.tsx:101-108`, `subscribe(depositRef, onUpdate): unsubscribe`).
- Produces: `createWsLedgerBridge(connection: Pick<PreviewBridgeConnection, "on">): LedgerBridge` — adapts the live WS connection's `ledger:update` and `turn:settled` subscriptions to the `LedgerBridge` seam (closes the "real WS bridge wiring" owner-gated note from Phase 13) — **and `createWsDepositSubscription(connection: Pick<PreviewBridgeConnection, "on">): DepositSubscription`** — folds `ledger:update` (deposit rows for the subscribed ref) + `deposit:failed` frames into `DepositUpdate`s (closes the P3 "deposit:failed web consumer" owner-gated hook). Task 5 injects it into `TopUp`.

- [ ] **Step 1: Failing test** — a fake connection records subscriptions; emitting a `ledger:update` event through the fake reaches the bridge handler with the same payload; unsubscribe propagates (and is idempotent — the seam contract); a `turn:settled` event likewise; a `deposit:failed` frame for the subscribed ref reaches the `DepositSubscription` listener as a failure update while frames for OTHER refs are ignored. Reuse the event fixtures from `tests/ledger/state.test.tsx` so payload shapes stay honest.
- [ ] **Step 2: FAIL → implement (thin adapters) → PASS. Gates.**
- [ ] **Step 3: Commit** — `git commit -m "feat(web): real ws ledger bridge and deposit subscription adapters"`.
...
- Consumes: `FileChangedEventSchema` payload (`@nyx/protocol` `events.ts:424-433` — `FileChangedPayloadSchema = { path: FilePathSchema, content: z.string() }` at `:427-431`; already defined, currently UNHANDLED server-side; anchors re-verified post-P3 — P3's `deposit:failed` addition shifted this region again); `ProjectStore.commit(projectId, request)` (`store.ts:102`; `CommitRequest` at `store.ts:57-60` is `{ author: FileAuthor; files: readonly FileWrite[] }` — the user-edit marker is `author: "user"`, the D59 source distinction); `getVersionHistory` (`store.ts:110`) returning `VersionSnapshot[]` (`store.ts:78-85`: `{ version, author, createdAt, files: HandoffFile[] }` — filter `author === "user"`); the per-project turn-active state in the coordinator (`TurnGate` interface at `coordinator.ts:278-286` with `isTurnActive` at `:285`, active-turn tracking near `:516`; handler registration site: the `.on("prompt:submit", …).on("test:results", …).on("compile:results", …).on("console:log", …)` chain at `coordinator.ts:1026-1058` — register `file:changed` alongside).
- Produces:
  - a `file:changed` handler: ownership-checked `ctx.projectId` (iron rule: never trust client ids), REJECTED with an error event while a turn is active for that project (FR-047/D60 — find the existing error-event idiom the router uses and reuse it), otherwise `store.commit(ctx.projectId, { author: "user", files: [{ path, content }] })` (the field is `author`, not `source` — `CommitRequest`, `store.ts:57-60`).
...
- [ ] **Step 2: Failing handler tests** — (a) idle project: `file:changed` commits exactly one version with `author: "user"` and echoes success; (b) active turn: no commit, error event emitted naming the turn lock; (c) cross-project spoof: payload for project B on a connection bound to project A → rejected, nothing committed (bind to `ctx.projectId`, ignore any payload project field).
...
**Grounding (P1 retro, review finding F1):** the FR-032 per-circuit coverage telemetry is information-free on real GREEN runs — the wire DTO carries FAILING test names only, so green ⇒ `failures: []` ⇒ an honest all-uncovered report (documented at `apps/server/src/turn/coordinator.ts:821` post-P2; tests assert the honest gap). The real fix was explicitly recorded for P2/P6 re-planning. P2's Task 1 DID land the additive protocol field — `passedNames?: string[]` on `TestResultsPayloadSchema` is merged (`@nyx/protocol` `events.ts:349`, P2 retro discovery) — so this task lands ONLY the two consumers. Skip only with a retro note.

**Files:**

- Modify: `apps/web/src/container/testrunner.ts` (emit passing test `fullName`s from the vitest JSON report's `assertionResults` where `status === "passed"` — the parser already walks them), `apps/server/src/agents/coverage.ts` (`testNamesFromResults` folds `passedNames` alongside failure names; `capTestResults` must cap the enlarged payload under the same FR-033 32KB wire frame — passing names are the FIRST thing truncated, failures keep priority). The protocol field itself is already merged (`events.ts:349`) — no protocol change needed.
...
- Consumes: `dirtyPaths`/`EditorState` (Task 9); the bridge `send` for `file:changed` and `deploy:request` (payload `{}` — `events.ts:417-421`, re-anchored post-P3) and `deploy:status` subscription; P2's `CompileWorkerClient.check(sources)` for Build — **surface verified at P3 Task 0**: `createCompileWorkerClient` from the `@/compile` barrel (`compile/client.ts:32-39`: `check(sources): Promise<CheckOutput>` / `compileFull` / `dispose`; `CheckOutput` at `compile/messages.ts:16`, `FullOutput` extends it at `:28`, both carrying `compilerVersion` + `durationMs` from `@nyx/compact-wasm`'s `CheckResult`/`CompileResult`); diagnostics → monaco markers via `monaco.editor.setModelMarkers`. Timing/version for the pitch chip come from the compile result itself — P2's `compile:results`/worker results carry `durationMs` and `compilerVersion` (plus optional `circuits`); render `durationMs` as seconds and show `compilerVersion` in the chip tooltip. Never measure with `Date.now()` around the call when the result already reports its own duration.
- Produces:
...
- **⚠️ P2 wiring OBLIGATION (recorded in the P2 retro, discharged HERE):** `createPreview(handle, bridge, deps)` registers the `compile:run` handler ONLY when BOTH `deps.projectId` AND `deps.getSources` are supplied (`container/preview.ts:64-80` — "when either is absent, compile handling is not wired"). The Workspace assembly MUST pass `projectId` + a `getSources` resolving the CURRENT editor/project sources (server state + saved drafts) — and share one `CompileWorkerClient` between the toolbar Build and the compile handler (`deps.compileWorker`) so two workers never race. Without this, server-delegated turn compiles silently never run.
